...
def decrypt_image(request):
    if request.method == 'POST':
        ...
        # Ensure s_box is a list, not a JSON string
        ...
        try:
            # Paste decrypted blocks onto the existing encrypted image
            ...
            decrypted_folder = os.path.join(settings.BASE_DIR, 'media')
            os.makedirs(decrypted_folder, exist_ok=True)
            decrypted_image_path = os.path.join(decrypted_folder, os.path.basename(encrypted_image_path))
            decrypted_image_path = decrypted_image_path.split('/media')[1]
            decrypted_image_path = 'media' + decrypted_image_path

            encrypted_image.save(decrypted_image_path)

            print("decrypted_image_path", decrypted_image_path)

            return JsonResponse({'success': True, 'decrypted_image_path': decrypted_image_path})
        except Exception as e:
